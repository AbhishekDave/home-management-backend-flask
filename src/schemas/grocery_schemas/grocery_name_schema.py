...
class GroceryNameSchema(Schema):
    ...
    def users(self):
        ...
        return fields.List(fields.Nested(CompleteUserSchema, many=True), dump_only=True)     # Provided in output (e.g., responses)

    @property
    def grocery_items(self):
        from src.schemas.grocery_schemas.grocery_item_schema import GroceryItemSchema
        return fields.List(fields.Nested(GroceryItemSchema, many=True), dump_only=True)
