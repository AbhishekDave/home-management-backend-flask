...
from src.services.user_services.user_retrieval_service import UserRetrievalService
...
def auth_requires_at_registration(f):
    ...
    def decorated(*args, **kwargs):
        ...
        user_retrieval_service = UserRetrievalService(db)
        user_data = request.get_json()
        if not user_data:
            raise MissingDataException('No input data provided.')

        user_by_username = user_retrieval_service.find_user_by_username(user_data['username'])
        user_by_email = user_retrieval_service.find_user_by_email(user_data['email'])
